...
PROJECT_PATH_STORAGE?=storage:comp

PROJECT_PATH_ENV?=/comp

##### JOB NAMES #####

PROJECT_POSTFIX?=comp
...
jupyter: upload-code upload-notebooks ### Run a job with Jupyter Notebook and open UI in the default browser
	$(NEURO) run \
		...
		--volume $(PROJECT_PATH_STORAGE)/$(RESULTS_DIR):$(PROJECT_PATH_ENV)/$(RESULTS_DIR):rw \
		$(CUSTOM_ENV_NAME) \
		'env PYTHONHASHSEED=42 jupyter notebook --no-browser --ip=0.0.0.0 --allow-root --NotebookApp.token= --notebook-dir=$(PROJECT_PATH_ENV)'
